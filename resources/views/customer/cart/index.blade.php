...
@section('content')
    @if ($cart->cart_items->isEmpty())
        <h5 class="mt-4">Stok produk menipis nih, Yuk beli sebelum kehabisan <a
                href="{{ route('main.produk-list') }}">Belanja
                Sekarang!</a></h5>
    @else
                                    ...
                                    <tbody>
                                        @foreach ($cart->cart_items as $item)
                                            <tr class="border-bottom">
                                                ...
                                                <td>
                                                    <div class="d-flex">
                                                        <form action="{{ route('customer.cart.delete-item', $item->id) }}"
                                                            method="post">
                                                            @csrf
                                                            @method('DELETE')
                                            ...
                                            <div class="modal-body">
                                                <div class="mb-2">
                                                    <form action="{{ route('customer.order.checkout') }}" method="post">
                                                        ...
                                                        <button type="submit" class="btn btn-outline-primary col-12"><i
                                                                class="bi bi-cash-stack me-2"></i>Tunai</button>
                                                    </form>
                                                </div>
                                                <div class="">
                                                    <form action="{{ route('customer.order.checkout') }}" method="post">
                                                        @csrf
                                                        <input type="hidden" name="pembayaran" value="Non-Tunai">
                                                        <button type="submit" class="btn btn-outline-primary col-12"><i
                                                                class="bi bi-wallet me-2"></i>Non
                                                            Tunai</button>
                                                    </form>
                                                </div>
                                            </div>
